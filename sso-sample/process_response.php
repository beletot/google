...
function signResponse($responseXmlString, $pubKey, $privKey) {
  ...
  $tempFileName = 'C:\wamp\www\google\sso-sample\\'. samlCreateId() . '.xml';
  ...
  if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {
    ...
    $cmd = 'C:\libs\xmlsec-win32\bin\xmlsec sign --privkey-pem ' . $privKey . 
	     ' --pubkey-der ' . $pubKey . ' --output ' . $tempFileName . 
	     '.out ' . $tempFileName;
	//error_log('CMD win -> '.$cmd);	 
  } else {
  ...
  //exec($cmd, $resp);
  if(exec($cmd, $resp)){
  	error_log(__LINE__.' error exec cmd');
  }else{
  	error_log(__LINE__.' okay exec cmd');
  }
  var_dump($resp);
  unlink($tempFileName);
  ...
  $requestXmlString = samlDecodeMessage($SAMLRequest);
    if (($requestXmlString == '')||($requestXmlString === FALSE)) {
      ...
      if ($username == '') {
        ...
        * responses for the epfc.eu domain.
        */
